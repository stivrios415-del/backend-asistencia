...
const notificarFalta = async (cedula, fecha, institucion_id) => {
  try {
    const { data: est } = await supabase
      .from('estudiantes')
      .select('nombre, apellido, grado, seccion')
      .eq('cedula', cedula)
      .maybeSingle();
    ...
    if (faltas >= 5) {
      tipo   = 'muchas_ausencias';
      titulo = '🔴 Alerta crítica de ausencias';
...
const actualizarPushToken = async (req, res) => {
  try {
    ...
    console.error('Error actualizarPushToken:', error.message);
...
const getMisNotificaciones = async (req, res) => {
  try {
    ...
    console.error('Error getMisNotificaciones:', error.message);
    return res.status(500).json({ error: 'Error interno' });
  }
};
