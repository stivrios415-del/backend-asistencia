...
const getAsistenciaEstudiante = async (req, res) => {
  try {
    ...
    let query = supabase
      .from('asistencia')
      .select(`id, fecha, hora, materias (id, nombre, grado, seccion, carrera)`)
...
const getAlertasEstudiante = async (req, res) => {
  try {
    ...
    if (!errAsis) {
      const faltas30 = 30 - (asistencias30 || 0);
      if (faltas30 >= 5) {
        alertas.push({
          id: `falta_${Date.now()}`,
          tipo: 'danger',
    ...
    if (alertasGuardadas) alertas.push(...alertasGuardadas);
...
const generarReporteEstudiante = async (req, res) => {
  try {
    ...
    let query = supabase
      .from('asistencia')
      .select(`fecha, hora, materias (id, nombre, grado, seccion, carrera)`)
      .eq('cedula', estudianteId)
      .gte('fecha', inicio)
      .lte('fecha', fin)
