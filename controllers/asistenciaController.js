...
const exportarReporteCompletoExcel = async (req, res) => {
  ...
  try {
    // 1. Obtener todos los estudiantes con filtros y orden
    let query = supabase
      .from('estudiantes')
    ...
    // 3. Generar Excel
    const workbook = new ExcelJS.Workbook();
    const fileNameSuffix = `${grado ? `_${grado}` : ''}${seccion ? `_sec${seccion}` : ''}`;
    const worksheet = workbook.addWorksheet(`Asistencia${fileNameSuffix}_${fecha}`);
    worksheet.columns = [
      { header: 'Cédula', key: 'cedula', width: 15 },
      { header: 'Nombre', key: 'nombre', width: 20 },
      { header: 'Apellido', key: 'apellido', width: 20 },
    ...
    // 4. Guardar buffer y subir a Storage
    const buffer = await workbook.xlsx.writeBuffer();
    const fileName = `reporte_completo${fileNameSuffix}_${fecha}_${Date.now()}.xlsx`;
    const { data: uploadData, error: uploadError } = await supabase.storage
      .from('reportes')
    ...
    // 6. Guardar metadata (opcional, agregar columnas grado/seccion si quieres auditoría)
    // Nota: Si agregaste columnas 'grado' y 'seccion' a reportes_generados, guárdalas; si no, ignora.
    try {
      await supabase.from('reportes_generados').insert([{
        fecha_inicio: fecha,
        fecha_fin: fecha,
      ...
      console.warn('⚠️ No se pudieron guardar los filtros en metadata (columnas pueden faltar)', metaErr.message);
    }
